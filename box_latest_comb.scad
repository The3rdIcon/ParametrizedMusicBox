...
/* to change shape of comb's soundboard, edit 3dplot.scad's z function -m. lau */
use <MCAD/involute_gears.scad>
use <write/Write.scad>
use <3dplot.scad>
// model config
// Is this to generate models for 3D printing or for the assembled view?
FOR_PRINT=0; // [0:Assembled, 1:PrintPlate]
...
// Should the Comb be generated?
GENERATE_COMB=1; // [1:yes, 0:no]
...
// http://www.wizards23.net/projects/musicbox/musicbox.html
//teethNotes="C 3C#3D 3D#3E 3F 3F#3G 3G#3A 3A#3B 3C 4C#4D 4D#4E 4F 4";
//teethNotes="C 4C#4D 4D#4E 4F 4F#4G 4G#4A 4A#4B 4C 5C#5D 5D#5E 5F 5";
//teethNotes="G#4A 4A#4B 4C 4C#4D 4D#4E 4F 4F#4G 4C 5C#5D 5D#5E 5F 5"; //this 
teethNotes="A#3A#3C 4C 4D 4D 4D#4D#4F 4F 4G 4G 4A 4";
...
boltHoleInnerRadius = 4;
boltHoleOuterRadius = 5 * 1.15;
...
//recursive function to check each of the teeth and return the max length one
//has to be one line like this because variable scoping is terrible
function findMaxTeethL(maxT_so_far,cur_index, end_index) = maxT_so_far < TeethLen(cur_index) ? (cur_index == end_index ? TeethLen(cur_index) : findMaxTeethL(TeethLen(cur_index),cur_index + 1, end_index)) : (cur_index == end_index ? maxT_so_far :findMaxTeethL(maxT_so_far,cur_index + 1, end_index));

maxTeethL = findMaxTeethL(0,0,notes_count - 1);
echo(maxTeethL);
amp_length = maxTeethL * 0.5;
...
negXEnd = -(noteExtendX*1.5+musicCylinderRadiusX);
...
function LengthOfTooth(f, h, E, ro) =
	sqrt((gammaTooth*gammaTooth*h/(4*PI*f))*sqrt(ro));
...
module BoltHole() {
	difference() {
	  hull() {
	    cylinder(teethHolderW *1.1, r = boltHoleOuterRadius*1.1);
	    translate([0,-teethHolderW,0])
			cube([teethHolderW, teethHolderW, teethHolderW]);
	  }
		cylinder(teethHolderW*1.1, r = boltHoleInnerRadius);
  }
}

// Generates comb and bolt holes
module MusicBox(plateNum) {
  // the comb
    ...
    if(plateNum == 0) {
        translate([-amp_length*0.66 - 25*0.33,  -gearBoxW * 2.61 - 3.5* epsilonCSG, -teethHolderH])
        mirror([0,0,0])
	rotate([0,0,180])
        intersection(){
    3dplot([0, 2],[1.6, 10],[10,10], -teethHolderH,[maxTeethL*1.5,musicH -teethHolderW ]);
            translate([-50,-50,0])
            cube([65,100,6]);
        }
    }
    if(plateNum == 1) {
    translate([0.34,  0, -teethHolderH])
	rotate([0,-10,180])
    cube([amp_length + 10, musicH + teethHolderW , teethHolderH]);
    }
  translate([2,boltHoleOuterRadius + gearBoxW - epsilonCSG,-teethHolderW * 0.7])
  BoltHole();
  translate([2,-boltHoleOuterRadius - teethHolderW - musicH + epsilonCSG,-teethHolderW* 0.7])
  mirror([0,1,0])
  BoltHole();
}
...
module slide() {
	difference() {
		hull() {
			cylinder(7, r = boltHoleOuterRadius);
			translate([negXEnd * 0.9,0,0])
			cylinder(7, r = boltHoleOuterRadius);
		}
		hull() {
			cylinder(8, r = boltHoleInnerRadius);
			translate([negXEnd * 0.9,0,0])
			cylinder(8, r = boltHoleInnerRadius);
		}
	}
...
// case shape
	if (GENERATE_CASE) {
		translate([0,FOR_PRINT?20:0,FOR_PRINT?-negXEnd*sin(cylinderGearTeethAngle):0])
		intersection() {
			if (FOR_PRINT) {
				maxX = max(posXEnd, -negXEnd);
				translate([0,0, 2*frameH+negXEnd*sin(cylinderGearTeethAngle)])
				cube([3*maxX, 2*frameW + boltHoleOuterRadius, 4*frameH], center=true);
			}
			rotate([FOR_PRINT?180:0, FOR_PRINT?-cylinderGearTeethAngle:0,0]) {
                ...
                //comb holding slide slots
                translate([-negXEnd - musicCylinderRadius, boltHoleOuterRadius + gearBoxW -epsilonCSG,-teethHolderW])
                {
                slide();
                translate([0,-2 * boltHoleOuterRadius - gearBoxW - teethHolderW - musicH + epsilonCSG,0])
                slide();
                }
        }
		// snapaxis for crank
...
if (GENERATE_PULLEY)
{
	...
	rotate([FOR_PRINT?180:-90,0,0])
	translate([crankLength,0,-pulleyH - 6])
	{
		// delta shaped end
...
if (GENERATE_COMB)
{
    rotate([FOR_PRINT?180:0,0,0]){
        mirror([0,0,0])
        translate([FOR_PRINT?50:0,FOR_PRINT?60:boltHoleInnerRadius*2+ wall_width, (FOR_PRINT?-negXEnd*sin(cylinderGearTeethAngle):0) ])
        rotate([FOR_PRINT?180:0, FOR_PRINT?-cylinderGearTeethAngle:0,0])
		translate([-(noteExtendX+musicCylinderRadiusX),-(gearHeight/2+gear_gap+teethGap) - 10, 5])
		rotate([0,-cylinderGearTeethAngle,0])
        {
            MusicBox(0);
            translate([0,2*gearHeight+wall_width,-teethHolderH]) cube([-negXEnd * 0.18,teethHolderW,teethHolderH]);
        }
    }
}